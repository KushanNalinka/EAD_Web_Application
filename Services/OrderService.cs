...
namespace EADWebApplication.Services
{
    public class OrderService
    {
        ...
        public OrderService(IOptions<MongoDBSettings> mongoSettings, NotificationService notificationService, CancelNotificationService cancelNotificationService, ProductService productService, ShortNotificationService shortNotificationService)
        {
        ...
        public async Task MarkOrderDeliveredAsync(string orderId)
        {
            ...
            // Notify the customer that their order has been dispatched and delivered using ShortNotificationService
        ...
        public async Task<bool> UpdateOrderItemStatusAsync(string orderId, string productId, string vendorEmail, int newStatus)
        {
            ...
            {
                // After updating the order item status, check if all items are now dispatched
                await UpdateOrderAndCheckStatusAsync(orderId);
            }
        ...
        // Method to update the order and check the status of the items
        public async Task UpdateOrderAndCheckStatusAsync(string orderId)
        {
            var order = await GetOrderByIdAsync(orderId);
            if (order == null) return;

            // Check if all items have the status '2' (Completed)
            bool allItemsDispatched = order.OrderItems.All(item => item.OrderItemStatus == 2);

            if (allItemsDispatched)
            {
                // Update order status to Dispatched (1) and Delivered to true
                order.OrderStatus = 2;
                ...
                await _shortNotificationService.CreateNotificationAsync(
                    order.UserId,
                    order.Email,
                    "Your order has been dispatched and delivered.",
                    order.Id);
            }
        }
